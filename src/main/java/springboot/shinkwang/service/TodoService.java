...
public class TodoService {
    ...
    public List<TodoEntity> update(final TodoEntity entity) {
        ...
        // 2) 넘겨받은 엔티티 id 를 이용해 TodoEntity 를 가져온다. 존재하지 않는 엔티티는 업데이트할 수 없기 때문이다.
        ...
        // 5) Retrieve Todo 에서 만든 메서드를 이용해 사용자의 모든 Todo 리스트를 리턴한다.
        return retrieve(entity.getUserId());
    }

    /** 삭제 */
    public List<TodoEntity> delete(final TodoEntity entity) {
        // 1) 삭제할 엔티티가 유효한지 확인한다.
        validate(entity);
        ...
        try {
            ...
            // 4) 컨트롤러로 exception 을 보낸다.
            // 데이터베이스 내부 로직을 캡슐화하려면 e 를 리턴하지 않고 새 exception 오브젝트를 리턴한다.
